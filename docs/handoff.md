...
| 5 | **Add perry lk2nd device node** | 🟡 **Built + binary-verified 2026-07-20; flash pending.** Patch `pmos/lk2nd/0001-*` adds `motorola-perry` to `msm8917-mtp.dts` (mirrors nora/hannah); `scripts/pmos-apply-lk2nd-perry.sh` builds it (lk2nd **r3**, perry strings + `dtb-files` confirmed embedded). Remaining = device-side flash to `boot` + on-device check — **gated runbook in [`pmos-lk2nd-perry-node.md`](pmos-lk2nd-perry-node.md)** (needs physical fastboot). Fixes `fdtdir` resolution + "Unknown (FIXME!)". |
